...
#Simple imputer
imp_mean = SimpleImputer(missing_values=np.nan, strategy='mean') # среднее
fit_mean = imp_mean.fit_transform(X)
res_mean = round_not_num (X, fit_mean, types_bytes)

#k-NN
imp_knn = KNNImputer(n_neighbors=2, weights="uniform")
fit_knn = imp_knn.fit_transform(X)
res_knn = round_not_num (X, fit_knn, types_bytes)
...
mean = np.zeros((8, nc))
var = np.zeros((8, nc))
pc_mean = np.zeros((8, nc))
pc_var = np.zeros((8, nc))
rmse = np.zeros(7)

rmse[0] = ma.sqrt(mean_squared_error(X_truth, res_mean))
rmse[1] = ma.sqrt(mean_squared_error(X_truth, res_knn))
rmse[2] = ma.sqrt(mean_squared_error(X_truth, res_em))

for i in range (0, nc):
    mean[0, i] = statistics.mean(X_truth[:, i])
    mean[1, i] = statistics.mean(res_mean[:, i])
    mean[2, i] = statistics.mean(res_knn[:, i])
    mean[3, i] = statistics.mean(res_em[:, i])
    mean[4, i] = statistics.mean(res_del.iloc[:, i])

    var[0, i] = statistics.variance(X_truth[:, i])
    var[1, i] = statistics.variance(res_mean[:, i])
    var[2, i] = statistics.variance(res_knn[:, i])
    var[3, i] = statistics.variance(res_em[:, i])
    var[4, i] = statistics.variance(res_del.iloc[:, i])
